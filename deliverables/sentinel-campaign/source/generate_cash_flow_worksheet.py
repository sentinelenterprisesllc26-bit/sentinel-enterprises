...
def _write_scenario(wb: xlsxwriter.Workbook) -> None:
    ...
    ws.write("A10", "Optional checkout → paid-order conversion rate assumption", f["body"])
    ...
    ws.write("A11", "Illustrative checkout clicks required at optional checkout → paid-order rate", f["body"])
    _formula(ws, 10, 1, '=IF(OR(B10="",B10<=0),"",B7/B10)', f["scenario_output_blank"])
    ws.write("C11", "Formula", f["formula"])
    ws.write("D11", "Blank until the optional rate is entered; this is checkout clicks required, not a website visitor count or actual result.", f["note"])
    ws.write("A13", "Guardrail", f["section"])
    ws.merge_range("A14:E15", "Do not use this scenario to backfill actual orders, checkout clicks, visitors, conversion, revenue, or cash. The optional rate yields checkout clicks required only; it does not estimate website visitors. Actual paid orders come from sales records; purchases are not attributed from pageviews.", f["warning"])
    ws.set_row(14, 25)
    ws.set_row(15, 25)
    ws.freeze_panes(4, 0)
